...
const FlashcardSummary = ({ id, question, answer, deckId }: Props) => {
  return (
    ...
    <Link
      ...
      className="block p-5 bg-white dark:bg-slate-700/60 shadow-md border border-transparent dark:border-slate-700 rounded-md hover:border-slate-300 dark:hover:border-slate-600 transition-all cursor-pointer group"
    >
      <div className="flex gap-3 items-start">
        ...
        <div className="flex justify-center items-center h-10 w-10 rounded-md bg-brand-mint-light dark:bg-emerald-950/40 text-brand-mint dark:text-emerald-400 dark:group-hover:text-emerald-300 shadow-sm shrink-0 transition-colors">
          <CardSim size={20} />
        </div>
